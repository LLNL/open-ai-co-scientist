# trying to show progress, but not quite working yet.
...
import os
import datetime
from fastapi import FastAPI, HTTPException, responses, Request, BackgroundTasks
...
async def call_llm(prompt: str) -> str:
    ...
    try:
        completion = await client.chat.completions.create(
            model="openai/gpt-3.5-turbo-instruct",  # Or any other suitable model
...
async def call_llm_for_generation(prompt: str, num_hypotheses: int = 3) -> List[Dict]:
    """
...
async def call_llm_for_reflection(hypothesis_text: str) -> Dict:
    """
    ...
    Returns:
        Dict: A dictionary containing the review results, including novelty and
              feasibility assessments (HIGH, MEDIUM, or LOW), a comment, and a list of
              references.
    """
    prompt = (
...
async def run_pairwise_debate(hypoA: Hypothesis, hypoB: Hypothesis) -> Hypothesis:
    """
    ...
    Returns:
        Hypothesis: The winning hypothesis. If scores are tied, a winner is chosen
                  randomly.
    """

    def score(h: Hypothesis) -> int:
        """
        Calculates a numerical score for a hypothesis based on its novelty and
        feasibility reviews.
        ...
        Returns:
            int: The calculated score.  HIGH=3, MEDIUM=2, LOW=1, None=0.  The score is
                 the sum of the novelty and feasibility scores.
        """
    ...
    winner = (
        ...
        if scoreB > scoreA
        else random.choice([hypoA, hypoB])
    )
    logger.info(
...
async def update_elo(winner: Hypothesis, loser: Hypothesis, k_factor: int = 32):
    """
...
async def combine_hypotheses(hypoA: Hypothesis, hypoB: Hypothesis) -> Hypothesis:
    """
    ...
    Returns:
        Hypothesis: A new hypothesis combining the two input hypotheses. The new ID is
                  prefixed with "E".
    """
    ...
    logger.info(
        "Combined hypotheses %s and %s into %s",
        hypoA.hypothesis_id,
        hypoB.hypothesis_id,
        new_id,
    )
...
async def similarity_score(textA: str, textB: str) -> float:
    """
    Calculates a similarity score between two text strings.
    This is a placeholder. In a real implementation, this would use a more
    sophisticated similarity measure like cosine similarity or Jaccard index.
    ...
    Returns:
        float: A similarity score between 0 and 1 (inclusive). For now, it returns a
               random number.
    """
    score = random.uniform(0, 1)
    logger.info(
        "Similarity score between %s and %s: %f (placeholder)",
        textA,
        textB,
        score,
    )
...
class GenerationAgent:
    async def generate_new_hypotheses(
        ...
        raw_output = await call_llm_for_generation(prompt, num_hypotheses=2)
        new_hypos = []
...
class ReflectionAgent:
    async def review_hypotheses(
        self, hypotheses: List[Hypothesis], context: ContextMemory
    ) -> None:
        """
        Reviews a list of hypotheses, updating their novelty, feasibility, comments,
        and references.
        ...
        """
        for h in hypotheses:
            result = await call_llm_for_reflection(h.text)
...
class RankingAgent:
    async def run_tournament(
        ...
        """
        Runs a tournament among the given hypotheses, updating their Elo scores and
        recording results.

        Args:
            hypotheses (List[Hypothesis]): The list of hypotheses to participate in the
                                          tournament.
            context (ContextMemory): The current context memory.
        ...
        for hA, hB in pairs:
            if hA.is_active and hB.is_active:
                winner = await run_pairwise_debate(hA, hB)
                loser = hB if winner == hA else hA
                await update_elo(winner, loser)
                logger.info(
                    "Ran pairwise debate between %s and %s. Winner: %s",
                    hA.hypothesis_id,
                    hB.hypothesis_id,
                    winner.hypothesis_id,
                )
...
class EvolutionAgent:
    async def evolve_hypotheses(
        self, top_k: int, context: ContextMemory
    ) -> List[Hypothesis]:
        """
        ...
        Returns:
            List[Hypothesis]: A list of new, evolved hypotheses.  Currently, at most
                              one new hypothesis is generated by combining the top two.
        """
        ...
        if len(top_candidates) >= 2:
            new_h = await combine_hypotheses(top_candidates[0], top_candidates[1])
    ...
    async def build_proximity_graph(
        ...
        for i in range(len(hypotheses)):
            ...
            for j in range(len(hypotheses)):
                if i == j:
                    continue
                sim = await similarity_score(hypotheses[i].text, hypotheses[j].text)
...
class MetaReviewAgent:
    async def summarize_and_feedback(self, context: ContextMemory, adjacency: Dict) -> Dict:
        """
...
class SupervisorAgent:
    ...
    async def run_cycle(self, research_goal: ResearchGoal, context: ContextMemory) -> Dict:
        """
        ...
        # 1. Generation
        new_hypotheses = await self.generation_agent.generate_new_hypotheses(research_goal, context)
        for nh in new_hypotheses:
            context.add_hypothesis(nh)
        # 2. Reflection
        active_hypos = context.get_active_hypotheses()
        await self.reflection_agent.review_hypotheses(active_hypos, context)
        # 3. Ranking (Tournament)
        active_hypos = context.get_active_hypotheses()
        await self.ranking_agent.run_tournament(active_hypos, context)
        # 4. Evolution (Improve top ideas)
        new_evolved = await self.evolution_agent.evolve_hypotheses(top_k=2, context=context)
        for nh in new_evolved:
            context.add_hypothesis(nh)
        if new_evolved:
            await self.reflection_agent.review_hypotheses(new_evolved, context)
        # 5. Ranking again
        active_hypos = context.get_active_hypotheses()
        await self.ranking_agent.run_tournament(active_hypos, context)
        # 6. Proximity Analysis
        adjacency = await self.proximity_agent.build_proximity_graph(active_hypos, context)
        # 7. Meta-review
        overview = await self.meta_review_agent.summarize_and_feedback(context, adjacency)
...
def set_research_goal(goal: ResearchGoalRequest, background_tasks: BackgroundTasks):
    """
    ...
    # Run a cycle in the background after setting the research goal
    background_tasks.add_task(supervisor.run_cycle, current_research_goal, global_context)
...
async def run_cycle(request: Request):
    ...
    Returns:
        StreamingResponse: An SSE stream of progress updates and the final result.
    """
    global current_research_goal, global_context

    async def event_generator():
        if not current_research_goal:
            yield "data: No research goal set.\\n\\n"
            return
        
        try:
            loop = asyncio.get_event_loop()

            yield "data: Starting a new cycle...\\n\\n"
            await asyncio.sleep(0.1)

            # 1. Generation
            yield "data: Generating new hypotheses...\\n\\n"
            await asyncio.sleep(0.1)
            new_hypotheses = await supervisor.generation_agent.generate_new_hypotheses(
                current_research_goal, global_context
            )
            for nh in new_hypotheses:
                await loop.run_in_executor(None, global_context.add_hypothesis, nh)

            # 2. Reflection
            yield "data: Reviewing hypotheses...\\n\\n"
            await asyncio.sleep(0.1)
            active_hypos = await loop.run_in_executor(
                None, global_context.get_active_hypotheses
            )
            await supervisor.reflection_agent.review_hypotheses(
                active_hypos, global_context
            )

            # 3. Ranking (Tournament)
            yield "data: Running tournament...\\n\\n"
            await asyncio.sleep(0.1)
            active_hypos = await loop.run_in_executor(
                None, global_context.get_active_hypotheses
            )
            await supervisor.ranking_agent.run_tournament(active_hypos, global_context)

            # 4. Evolution (Improve top ideas)
            yield "data: Evolving hypotheses...\\n\\n"
            await asyncio.sleep(0.1)
            new_evolved = await supervisor.evolution_agent.evolve_hypotheses(
                top_k=2, context=global_context
            )
            for nh in new_evolved:
                await loop.run_in_executor(None, global_context.add_hypothesis, nh)
            if new_evolved:
                await supervisor.reflection_agent.review_hypotheses(
                    new_evolved, global_context
                )

            # 5. Ranking again
            yield "data: Running tournament again...\\n\\n"
            await asyncio.sleep(0.1)
            active_hypos = await loop.run_in_executor(
                None, global_context.get_active_hypotheses
            )
            ...
            # 6. Proximity Analysis
            yield "data: Building proximity graph...\\n\\n"
            await asyncio.sleep(0.1)
            adjacency = await supervisor.proximity_agent.build_proximity_graph(
                active_hypos, global_context
            )

            # 7. Meta-review
            yield "data: Performing meta-review...\\n\\n"
            await asyncio.sleep(0.1)
            overview_dict = await supervisor.meta_review_agent.summarize_and_feedback(
                global_context, adjacency
            )
            await loop.run_in_executor(
                None,
                lambda: setattr(
                    global_context, "iteration_number", global_context.iteration_number + 1
                ),
            )
            logger.info("Cycle complete, iteration now %d", global_context.iteration_number)
            ...
            response = OverviewResponse(
                ...
                suggested_next_steps=overview_dict["research_overview"][
                    "suggested_next_steps"
                ],
            )
        ...
        <script>
            ...
            async function runCycle() {
                document.getElementById("results").innerHTML =
                    "<p>Running cycle...</p><progress id='progress'></progress>";
                const eventSource = new EventSource("/run_cycle");

                eventSource.onmessage = function (event) {
                    const data = JSON.parse(event.data);

                    // Check if it's a progress update or the final result
                    if (data.iteration) {
                        // Final result
                        eventSource.close();
